...
mod test {
    ...
    fn test_find_table_xsdt() {
        ...
        // Build XSDT with 2 entries pointing into the same buffer
        let xsdt_entries = 2;
        let xsdt_len = sdt_hdr_size + xsdt_entries * 8;
        let facp_off = xsdt_len;
        ...
        buf[4..8].copy_from_slice(&(xsdt_len as u32).to_le_bytes());

        // FACP and MCFG table headers
        buf[facp_off..facp_off + 4].copy_from_slice(b"FACP");
        buf[mcfg_off..mcfg_off + 4].copy_from_slice(b"MCFG");

        // Use offsets as fake "addresses" — deref resolves them within buf
        buf[sdt_hdr_size..sdt_hdr_size + 8].copy_from_slice(&(facp_off as u64).to_le_bytes());
        buf[sdt_hdr_size + 8..sdt_hdr_size + 16].copy_from_slice(&(mcfg_off as u64).to_le_bytes());

        let rsdp = Rsdp {
            revision: 2,
            rsdt_address: 0,
            xsdt_address: 0, // XSDT starts at offset 0
        };
